...
export default function AnchorProperties() {
  ...
  const anchorId = selectedPoint.id as number;

  const anchor = anchorPoints[anchorId];
  if (!anchor) return <div className="text-xs text-gray-500">Anchor not found</div>;

  const isSnapped = !!anchor.snapPointId;
  const snappedTo = isSnapped ? snapPoints.find(sp => sp.id === anchor.snapPointId) : null;

  return (
    <>
      <button
        onClick={() => toggleAnchorCurve(anchorId)}
      ...
      <div>
        ...
        <input
          ...
          value={anchor.name || ''}
          onChange={(e) => {
            updateAnchorPoint(anchorId, cur => ({ ...cur, name: e.target.value }));
          }}
      ...
      {isSnapped && snappedTo && (
        <div className="bg-blue-900/30 border border-blue-700 rounded p-3 space-y-2">
          <div className="text-xs text-blue-300 font-medium">Snapped to: {snappedTo.name}</div>
          <button
            onClick={() => unsnapAnchor(anchorId)}
            className="w-full px-3 py-2 bg-blue-600 border-none rounded text-white cursor-pointer text-sm font-medium hover:bg-blue-700 transition-colors flex items-center justify-center gap-2"
          >
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => {
            ...
            updateAnchorPoint(anchorId, cur => ({ ...cur, position: { ...cur.position, x: val } }));
          }}
          disabled={isSnapped}
          className="w-full px-2 py-2 bg-gray-800 border border-gray-700 rounded text-gray-100 text-sm focus:outline-none focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
        />
      </div>
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => {
            const val = parseFloat(e.target.value || '0');
            updateAnchorPoint(anchorId, cur => ({ ...cur, position: { ...cur.position, y: val } }));
          }}
          disabled={isSnapped}
          className="w-full px-2 py-2 bg-gray-800 border border-gray-700 rounded text-gray-100 text-sm focus:outline-none focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
        />
          ...
          <div className="grid grid-cols-2 gap-2">
            <input
              ...
              onChange={(e) => {
                ...
                updateAnchorPoint(anchorId, cur => ({
                  ...cur,
